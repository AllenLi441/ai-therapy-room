...
export function buildCounselorSystemPrompt(input: {
  ...
  return [
    ...
    ...(input.pace === "fast"
      ? [
          "用 1-2 句简短回应：先用一句精准接住情绪（用具体的情绪词，落在对方的具体经历上），再给一个具体的小锚点或一个开放式的小问题；不铺满四步、不做机制分析、不堆叠建议。"
        ]
      : [
          "用 2-4 个短段落自然回应。结构必须是：",
          "1. 先精准接住情绪：用具体的情绪词命名对方此刻最核心的感受（参考督导的'本轮情绪判读'与'必须先反映'，但要用自己的话、落在对方的具体经历上）；不确定时用试探语气确认，给对方纠正空间。",
