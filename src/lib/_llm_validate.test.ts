...
// Body-emergency red flags — want suggested_flags to include medical_red_flag for
// the first two (acute), NOT the third (benign somatic). Validates the ② classifier
// change. These are not suicide intercepts, so intercept=n is expected/correct.
const MED = [
  "我突然胸口剧痛，喘不上气，话也说不清。",
  "这两天一直胸闷、有点喘，第一次这样。",
  "最近老头疼，可能没睡好。"
...
async function run(text: string) {
  ...
  const sev = outcome.kind === "ok"
    ? `${outcome.result.severity}/${outcome.result.pragmatic}/${outcome.result.confidence} flags=[${outcome.result.suggestedFlags.join(",")}]`
    : outcome.kind;
...
describe("LLM layer live validation", () => {
  it("runs representative cases", { timeout: 240000 }, async () => {
    ...
    lines.push("", "### MED (want flags=[medical_red_flag] for first two, NOT the third):");
    for (const t of MED) lines.push(await run(t));
