...
export async function POST(request: Request) {
  ...
  async function respondTailoredCrisis(
    ...
    const headers: Record<string, string> = {
      ...
      // 2026-07-08 owner directive: the appended in-text resource block is gone —
      // hotlines/1–4 check-in live ONLY in the always-on UI (crisis banner tel: links,
      // CrisisSheet, global footer). X-Crisis-Mode lets the client/eval keep the
      // crisis-vs-concern distinction without text fingerprints.
      "X-Crisis-Mode": mode
    };
  ...
  // 危机会话中的 1–4 数字确认回复:确定性解读,不交给模型自由发挥。
  // (闭环 2026-06-13 恢复时断线:解析器存在但无调用方;此处接回。)
  // 2026-07-05(D1/D4 按钮化后):资源块文字不再必然含「一个数字」的问句——那句问话
  // 现在是危机横幅下的 1–4 按钮,按钮发送的是裸数字,与用户手动打字等价。所以这里始终
  // 传 assumeAsked:true,让 classifyCrisisCheckReply 在 crisisModeActive 期间照常解析
  // 裸数字回复,不依赖 lastAssistant 的文字是否包含问句。
  if (crisisModeActive) {
    ...
    if (check) {
      ...
      return new Response(textStreamFromString(createCrisisReplyResponse(check.tier, language)), {
        headers: {
          ...
          "Cache-Control": "no-store",
          "X-Crisis-Triggered": "1",
          "X-Crisis-Source": `check_reply_${check.tier}`
        }
      });
    }
  }
  ...
  // Soft-cue front gate (same ≤4-message window as assessConversationRisk): a user
  // should never see a normal-looking reply for a few seconds before a danger
  // intervention shows up behind it. If ANY implicit-suspicious phrase is present in
  // the recent window, downgrade this turn from the parallel fast path to the
  // blocking judge flow below — the model still answers at fast-tier speed, only the
  // judge stops being concurrent.
  const recentUserConcat = messages
    .filter((m) => m.role === "user")
    .slice(-4)
    .map((m) => m.content)
    .join("\n");
  if (
    resolveSessionPace(body.pace) === "fast" &&
    lexiconClean &&
    !hasImplicitSuspicion(recentUserConcat) &&
    !crisisModeActive &&
    !body.exitedCrisis
  ) {
    ...
    const judgePromise = assessImplicitRiskWithLLM(messages, 5_000, 4_000)
    ...
    const resolveSafety = async () => {
      ...
      if (d.intercept && d.mode === "crisis") {
        return { event: { type: "safety", status: "crisis" } }; // banner (not text) carries hotlines — owner directive 2026-07-08
      }
      if (d.intercept && d.mode === "suicide_concern") {
        return { event: { type: "safety", status: "suicide_concern" } };
      }
  ...
  const webResults =
    infoSeeking && resolveSessionPace(body.pace) === "deep" && !crisisModeActive && knowledge.length === 0
      ? await searchAuthoritative(knowledgeQuery, 3)
      : [];
