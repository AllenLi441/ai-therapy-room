...
0.9.0 更新一般信息 RAG：新增 40 张有来源及适用边界的知识卡，信息提问采用独立生成提示，修复短追问、倾诉误检索、无证据数字补写和部分医疗/危机改述。知识卡的来源核对不等于专业批准。原 500 条训练任务与数据不属于本次网站发布。

0.8.2 将支持资源扩展为 15 个明确地区及「其他地区」，英国与爱尔兰分别选择，旧的组合设置保留兼容。页面、危机确认回复、未成年人提示、备份和请求校验共用地区定义，切换语言不会改变所选地区。资源卡片提供机构来源和必要的服务限制。
...
```sh
npm run release:check -- https://ai-therapy-room.vercel.app 0.9.0 EXPECTED_GIT_COMMIT
```
...
```sh
docker build --build-arg APP_BUILD_COMMIT=EXPECTED_GIT_COMMIT -t jingshi:0.9.0 .
docker run -d --name jingshi-candidate --restart unless-stopped \
  --env-file /etc/jingshi/runtime.env \
  -p 127.0.0.1:3001:3000 jingshi:0.9.0
npm run release:check -- http://127.0.0.1:3001 0.9.0 EXPECTED_GIT_COMMIT
```
