...
```sh
npm run release:check -- https://ai-therapy-room.vercel.app 0.8.2 EXPECTED_GIT_COMMIT
```
...
```sh
docker build --build-arg APP_BUILD_COMMIT=EXPECTED_GIT_COMMIT -t jingshi:0.8.2 .
docker run -d --name jingshi-candidate --restart unless-stopped \
  --env-file /etc/jingshi/runtime.env \
  -p 127.0.0.1:3001:3000 jingshi:0.8.2
npm run release:check -- http://127.0.0.1:3001 0.8.2 EXPECTED_GIT_COMMIT
```
